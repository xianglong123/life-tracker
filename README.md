...
| 🤖 **AI 日报** | DeepSeek 自动生成中文日报，总结你今天做了什么 |
...
python3 main.py summary        # 生成今日日报
...
- 截图按日期归档，30 天后自动清理
- 不记录浏览器标签页 URL（隐私过滤）
- 唯一网络请求是调用 DeepSeek API 生成日报（仅发送活动摘要文本）
...
- Python 3.10+
- Flask（自动安装）
- DeepSeek API Key（从 `~/.hermes/.env` 读取）
